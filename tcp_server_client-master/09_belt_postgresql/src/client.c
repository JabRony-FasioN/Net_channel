...
void ReadFromServer(SSL* ssl) {
    char buf[1024];
    int bytes;
    while(1) // Запустим бесконечный цикл чтения данных
    {
        bzero(buf, sizeof(buf)); // очищаем буфер
...
//    {   char *msg = "psql -h localhost -p 5432 -U postgres -d mydb -c \"select * from temp_name;\"";
    {   char *msg = "psql 127.0.0.1 5432 postgres mydb \"SELECT * FROM temp_name\"";
//    {   char *msg = "psql 127.0.0.1 5432 postgres mydb \"INSERT INTO temp_name (Date, ID, FIO, Test) VALUES ('2023-10-12','2','Petrov Petr Petrovich','test server')\"";
