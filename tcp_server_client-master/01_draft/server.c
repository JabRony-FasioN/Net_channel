...
static int openssl_setup_compression(SSL_CTX *ctx, char *method)
{
    ...
    * default malloc/free instead of the ones passed to OpenSSL. Should malise
...
cont_out:
   ...
   /* It seems that OpenSSL-1.1.1 presets the mode differently.
      Without correction malise might hang in SSL_read() */
   {
      long mode = 0;
...
/* delete all environment variables whose name begins with malise_OPENSSL_
...
/* read in the "name" information (from field "issuer" or "subject") and
   ...
   malise_OPENSSL_X509_COMMONNAME
*/
