...
void UserToServerInteraction(SSL* ssl, const char* hostname, const char* port, const char* username, const char* dbname) {
    ...
    char buf_sql[1024];
    while (1) {
        printf("\nEnter SQL command (type exit to quit): ");
        fgets(buf_sql, sizeof(buf_sql), stdin);
        buf_sql[strcspn(buf_sql, "\n")] = 0;  // remove newline character at the end
        if (strncmp(buf_sql, "exit", 4) == 0) {
            break;
        }
        snprintf(buf, sizeof(buf), "psql %s %s %s %s \"%s\"", hostname, port, username, dbname, buf_sql);
...
int main(int count, char *strings[])
{
    ...
    ctx = InitCTX();
    char CertFile[] = "/home/dibianv/app/cert/client.pem";
    char KeyFile[] = "/home/dibianv/app/cert/client.key";
    char CAFile[] = "/home/dibianv/app/cert/root.crt";
