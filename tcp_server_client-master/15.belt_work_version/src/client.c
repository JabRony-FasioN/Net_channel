...
void UserToServerInteraction(SSL* ssl, const char* hostname, const char* port, const char* username, const char* dbname) {
    ...
    char *ptr = missing_rows;
    char *line = strsep(&ptr, "\n");
    while (line != NULL) {
        char *quoted_and_grouped_row = add_quotes_and_group_values(strdup(line));
        snprintf(buf, sizeof(buf), "psql 127.0.0.1 5432 postgres mydatabase \"INSERT INTO temp_name (Date, ID, FIO, Test) VALUES %s\"", quoted_and_grouped_row);
        SSL_write(ssl, buf, strlen(buf));   /* encrypt & send message */
        free(quoted_and_grouped_row);
        line = strsep(&ptr, "\n");
    }
